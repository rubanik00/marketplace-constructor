...
// @title Marketplace Contract with fee logic extension
// @notice standart Marketplace function with platform fee.
    ...
    ERC721Holder
{
    ...
    constructor(uint32 _fee, string memory _name, string memory _version) {
        __Signature_init(_name, _version);
        _setFee(_fee);
    }

    /// @dev Set new fee
    /// @param newFee - new fee amount in percents
    ...
    /// @param tokenAddress token address that will be transfered from contract to receiver address
    /// @param _to receiver address

    function unlockTokens(IERC20 tokenAddress, address _to) external onlyOwner {
        _unlockTokens(tokenAddress, _to);
    }
    ...
    function purchase(
        ...
        if (offer.saleToken == address(0)) {
            require(amount == msg.value, "Value is not equal to price.");
            ...
            IERC20(offer.saleToken).safeTransferFrom(
                msg.sender,
                offer.creator,
                amount - _calculatePart(amount) - royaltyAmount
            );
        }
