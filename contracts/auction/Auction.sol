...
import "./IAuction.sol";
import "../marketplace/extensions/Payout2981Support.sol";
    ...
    ReentrancyGuard
{
    ...
    {
        return (
            _auctionLots[auctionId].lastBidder,
            _auctionLots[auctionId].lastBid
        );
    }
    ...
    function addBid(uint256 auctionId, uint256 amount)
        external
        payable
        nonReentrant
    {
        ...
        require(
            ...
            "Last bidder cannot add bid again"
        );
        require(
            block.timestamp < auction.endTime ||
                auction.lastBid != auction.buyNowPrice,
            "Auction is already finished"
        );
        require(
            amount <= auction.buyNowPrice,
            "Cannot be more than fixed price"
        );

        if (auction.paymentToken == address(0)) {
            require(msg.value == amount, "Wrong msg.value");
        } else {
            uint256 balanceBefore = IERC20(auction.paymentToken).balanceOf(
                address(this)
            );
            ...
            uint256 balanceAfter = IERC20(auction.paymentToken).balanceOf(
                address(this)
            );
        ...
        if (auction.lastBidder != address(0)) {
            require(
                amount >= auction.lastBid + auction.delta,
            ...
            require(
                ...
                "Should be bigger than startPrice"
            );
        }
    ...
    function extendActionLifeTime(uint256 auctionId, uint256 newEndTime)
        public
    {
        AuctionLot memory auction = getAuctionInfo(auctionId);
        ...
        require(
            newEndTime - _auctionLots[auctionId].endTime <= 30 days,
            "Could extend only for 30 days"
        );
    ...
    function claimLot(uint256 auctionId) external payable {
        ...
        if (
            IERC165(auction.tokenContract).supportsInterface(
                type(IERC721).interfaceId
            )
        ) {
            if (
                IERC721(auction.tokenContract).supportsInterface(
            ...
            if (
                IERC1155(auction.tokenContract).supportsInterface(
                    type(IERC2981).interfaceId
                )
            ) {
                if (auction.paymentToken == address(0)) {
    ...
    function payoutPendingPayments(
        uint256 auctionId,
        address from,
        address to
    ) external onlyOwner {
